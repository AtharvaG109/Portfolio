...
export const hero = {
  ...
  summary:
    "From clinical iPad workflows and market research systems to cyber ranges and encrypted vaults, I turn complex domains into local-first products with visible safety boundaries and executable proof.",
  mission:
  ...
  utilityLinks: [
    ...
    { label: "Recent work", value: "ProsthPlan, Global Market Terminal, BreachOps, and FocusForge", href: "/projects/" },
...
export const stats = [
  ...
  {
    value: "251",
    label: "Passing tests in the Global Market Terminal compliance-first research system"
  },
  {
    value: "19",
...
export const roleFitCards = [
  {
    ...
    signal: "Evidence shown through ProsthPlan, Global Market Terminal, BreachOps, FocusForge, and SpectreFS."
  },
  {
...
const projectRecords = [
  {
    ...
    summary:
      "Built an iPadOS prosthodontic workflow and decision-support app with patient-first navigation, local clinical records, 3D scan tools, deterministic planning aids, immutable signed documents, and explicit doctor-confirmation gates.",
    impact:
      "Turned a broad clinical feature set into one coherent iPad workstation while keeping the safety boundary visible: suggestions remain drafts, doctors make clinical decisions, original files stay untouched, and signed records are corrected through addenda.",
    challenge:
      "Clinical software must compress a large, safety-sensitive workflow without hiding provenance, overwriting evidence, or presenting deterministic assistance as autonomous diagnosis.",
    approach: [
      "Built the app in SwiftUI and SwiftData around patient-first navigation, case-scoped workspaces, local audit history, versioned records, and app-managed copies that preserve imported originals.",
      "Implemented deterministic Kennedy and PDI-style classification support, treatment-option drafts, rule-based clinical suggestions, and material guidance with mandatory doctor review and confirmation.",
      "Added 3D scan import and viewing, mm-space measurements, annotations, CAD-lite geometry, guided margin detection, lab export, image annotation, and 2D smile-design tools.",
      "Added immutable signed documents, addenda, encrypted local backup, consult-mode filtering, scheduling, fee quotes, accessibility coverage, and clearly marked demo data."
    ],
    result:
    ...
    outcomes: [
      "Eight case tabs replace tool sprawl with a patient header, clinical workspace, responsive inspector, chairside focus, and direct routes from missing-record or review blockers.",
      "Clinical records preserve audit and version history; signed documents are immutable, corrections use addenda, and generated text remains draft-only until doctor approval.",
    ...
    architecture: [
      "Thin SwiftUI views bind to screen-focused ViewModels while services own import, storage, classification, planning, geometry, export, PDF, and audit logic.",
      "SwiftData models keep patient, case, media, scan, plan, consent, document, and audit records local with version-aware safety rules.",
      "Deterministic clinical services produce explainable drafts and warnings; the doctor-confirmation layer controls what becomes an approved clinical record."
    ],
    tradeoffs: [
      "Kept the MVP local-first instead of adding cloud sync so privacy, file custody, and audit behavior stay inspectable.",
      "Scoped DICOM to metadata and retained references rather than claiming diagnostic rendering, measurements, or window and level controls.",
      "Labeled CAD geometry investigational and kept millable crown export out of scope until thickness, undercut, watertightness, and independent clinical validation exist."
    ],
    stack: ["Swift", "SwiftUI", "SwiftData", "SceneKit", "PencilKit", "VisionKit", "XCTest"],
    media: {
      alt: "Product-flow diagram showing ProsthPlan patient records, deterministic clinical support, doctor confirmation, and immutable outputs."
    }
  },
  {
    ...
    summary:
      "Built a local, zero-cost Indian-market research terminal that reads user-owned NSE and BSE files in place, persists only derived artifacts, and refuses automation or model promotion without explicit data, licence, and validation gates.",
    impact:
      "Reframed a live-market idea into a defensible daily research product: bounded memory, derived-only storage, traceable provenance, explicit abstention, and no broker or order-routing path.",
    challenge:
      "The product had to be useful every day without quietly scraping restricted sources, retaining reconstructable price history, or presenting unvalidated research output as investment advice.",
    approach: [
      "Implemented streaming NSE and BSE imports, point-in-time Nifty 500 identity, bounded one-issuer series processing, and storage guards that reject raw or reconstructable price histories.",
      "Built post-close, pre-open, weekend, catch-up, and waiting workflows around official file availability, Asia/Kolkata cutoffs, process locks, and fail-closed data-quality checks.",
      "Added price-only research baselines, purged walk-forward validation, calibration, DSR and PBO gates, negative controls, model registry evidence, and abstention when inputs do not qualify.",
      "Kept acquisition deny-by-default with official-host pinning, no redirects, operator-supplied URL templates, explicit licence approval, and no built-in exchange scraper."
    ],
    result:
      "Version 0.2.1 records 251 passing tests. Deterministic benchmarks audited 1,229 stored JSON values with zero raw bars persisted; live capacity and forecast accuracy remain explicitly unclaimed.",
    outcomes: [
      "FastAPI, a terminal-dense browser UI, CLI commands, accessible local issuer autocomplete, and a macOS LaunchAgent cover the daily operator workflow.",
      "Paper-ledger entries require user confirmation, model outputs begin unvalidated, and no order-routing code exists.",
      "Every derived artifact carries provenance while missing files, questionable mappings, unresolved actions, drift, or leakage blockers force waiting or abstention."
    ],
    metrics: [
      { value: "251 tests", label: "Current local suite across storage, research, acquisition, API, and UI contracts" },
      { value: "0 raw bars", label: "Persisted in the measured derived-storage benchmark" },
      { value: "95% gate", label: "Minimum usable NSE and BSE mapping coverage for production activation" }
    ],
    architecture: [
      "External user-owned files feed streaming parsers and point-in-time identity resolution without being copied, modified, extracted, or deleted.",
      "Bounded calculation paths emit derived artifacts, validation evidence, model-registry rows, and confirmed paper-ledger records into local SQLite.",
      "FastAPI, CLI, browser terminal, and launchd scheduling all read the same derived contracts and explicit source-status state."
    ],
    tradeoffs: [
      "Accepted repeated external-file scans in the literal one-issuer daily path to preserve the tight raw-bar memory rule.",
      "Kept research training price-only and forced unsupported feature families to abstain instead of filling gaps with synthetic confidence.",
      "Left automated exchange access disabled unless the operator has current authorization and explicitly supplies it."
    ],
    stack: ["Python", "FastAPI", "Pydantic", "SQLite", "HTTPX", "Pytest", "Vanilla JavaScript"],
    media: {
      alt: "System-flow diagram showing official market files, bounded research calculation, validation gates, derived storage, and local terminal outputs."
    }
  },
  {
...
const projectSystemMaps = {
  "prosthplan-clinical-workstation": {
    lens: "Clinical decision path",
    title: "From source record to doctor-approved output",
    summary:
      "ProsthPlan keeps imported evidence, deterministic assistance, clinical judgment, and signed records in separate, reviewable layers.",
    nodes: [
      {
        id: "record",
        label: "Record",
        role: "Source",
        detail: "Patient, case, media, scan, and clinical records remain local; imported originals are preserved.",
        x: 14,
        y: 30
      },
      {
        ...
        label: "Assist",
        role: "Draft",
        detail: "Deterministic services derive explainable classifications, warnings, measurements, and planning drafts.",
        x: 38,
        y: 66
      },
      {
        ...
        label: "Confirm",
        role: "Doctor",
        detail: "A clinician reviews evidence, changes or rejects drafts, and explicitly confirms every clinical decision.",
        x: 62,
        y: 30
      },
      {
        ...
        label: "History",
        role: "Output",
        detail: "Approved records keep audit and version history; signed documents remain immutable and corrections use addenda.",
        x: 84,
        y: 66
      }
    ],
    edges: [
      { from: "record", to: "assist", label: "structured case facts" },
      { from: "assist", to: "confirm", label: "draft + rationale" },
      { from: "confirm", to: "history", label: "approved decision" }
    ]
  },
  "global-market-terminal": {
    lens: "Bounded research path",
    title: "From official file to qualified research output",
    summary:
      "Every step is built around provenance, bounded raw-data handling, explicit abstention, and operator control.",
    nodes: [
      {
        id: "files",
        label: "Files",
        role: "Source",
        detail: "User-owned official NSE and BSE files are opened read-only and validated without copying or mutation.",
        x: 14,
        y: 30
      },
      {
        ...
        label: "Stream",
        role: "Compute",
        detail: "One-issuer bounded series feed point-in-time features, research baselines, and data-quality checks.",
        x: 38,
        y: 66
      },
      {
        ...
        label: "Gate",
        role: "Validation",
        detail: "Coverage, drift, leakage, calibration, DSR, PBO, and promotion rules force waiting or abstention.",
        x: 62,
        y: 30
      },
      {
        ...
        label: "Terminal",
        role: "Output",
        detail: "CLI, API, and browser surfaces read derived artifacts and require confirmation for paper-ledger entries.",
        x: 84,
        y: 66
      }
    ],
    edges: [
      { from: "files", to: "stream", label: "validated rows" },
      { from: "stream", to: "gate", label: "derived evidence" },
      { from: "gate", to: "terminal", label: "qualified or abstained output" }
    ]
  },
...
const projectProofMetadata = {
  "prosthplan-clinical-workstation": {
    ...
    focusTags: ["SwiftUI", "SwiftData", "Clinical Workflow", "3D Geometry", "Audit", "Accessibility"],
  ...
  "global-market-terminal": {
    displayPriority: 2,
    track: "Product Engineering",
    maturity: "Validated local research terminal",
    roleFits: ["Backend Engineering", "Product Engineering", "Data Systems"],
    focusTags: ["Python", "FastAPI", "SQLite", "Point-in-Time Data", "Validation", "Compliance"],
    proofBadges: ["251 tests", "Zero raw bars stored", "Deny-by-default acquisition", "Explicit abstention", "Local-first", "No order routing"],
    outcomeLine:
      "Built a 251-test market research system that keeps raw bars external, stores only derived evidence, and refuses unlicensed or unvalidated automation.",
    readIf:
      "Read this if you care about data products where compliance, provenance, bounded storage, and honest uncertainty shape the architecture.",
    proofLine:
      "251 tests; derived-only SQLite; zero raw bars persisted in measured benchmarks; promotion and acquisition fail closed.",
    evidenceVisual: {
      title: "Global Market Terminal validation",
      label: "Research-system proof",
      lines: [
        "$ pytest -q",
        "251 passed",
        "stored raw bars: 0",
        "unqualified model output: abstain"
      ],
      caption: "Benchmarks prove implementation behavior, not forecast accuracy, live capacity, or investment return."
    }
  },
  "breachops-cyber-range": {
    displayPriority: 3,
...
export const nowSnapshot = {
  ...
  building: [
    ...
    "Hardening Global Market Terminal around authorized sources, point-in-time validation, abstention, and derived-only storage.",
  ...
  studying: [
    ...
    "Research-system validation: leakage control, calibration, data drift, and honest abstention.",
