...
export const hero = {
  ...
  utilityLinks: [
    ...
    { label: "Recent work", value: "SecureRAG EvalOps, uProxy, tinyc, Enterprise NIDS, Patchbot, Sysguard, and systems security projects", href: "/projects/" },
...
const projectRecords = [
  {
    ...
    summary:
      "Built a private-deployment RAG evaluation platform with offline-first ingestion, hybrid retrieval, citation validation, guardrails, audit logging, and production hardening.",
    impact:
    ...
    result:
      "SecureRAG EvalOps now ships as a public portfolio release and internal-production baseline with a full quality gate, reproducible migrations, local demo corpus, web UI, and deployment guidance.",
    outcomes: [
      ...
      "Supports document collections, retention cleanup, namespace-scoped access, metrics, traces, guardrails, and eval runs through one backend.",
      "Ships with production deployment docs, API-key bootstrap, audit logging, CI, and 73 passing tests."
    ],
    metrics: [
      { value: "73 tests", label: "Full release gate across auth, retrieval, eval, guardrails, and UI" },
      { value: "0 API keys", label: "Required for the default offline local workflow" },
      { value: "9-step query flow", label: "Authz through redaction with fail-closed citations" }
    ],
    architecture: [
      "Documents flow through parsers, structure-aware chunking, deterministic local embeddings, and Qdrant payloads scoped by namespace.",
      "Queries pass through authorization, guardrails, hybrid retrieval, MMR reranking, citation-grounded generation, validation, and redaction.",
      "Postgres persists metadata, evals, guardrail rows, audit events, and cost records while Redis caches query embeddings."
    ],
    tradeoffs: [
...
const projectProofMetadata = {
  "securerag-evalops": {
    ...
    proofBadges: ["Public repo", "CI gates", "Offline mode", "Audit logs", "Dashboard"],
    proofLine:
      "Offline-first RAG backend with fail-closed citations, guarded retrieval, audit logs, and production deployment docs.",
    evidenceVisual: {
      ...
      lines: [
        "$ poetry run pytest -q",
        "73 passed",
        "auth + citations + guardrails + audit",
  ...
  "uproxy-cxx-reverse-proxy": {
    displayPriority: 3,
    maturity: "Systems release",
    roleFits: ["Systems Engineering", "Backend Infrastructure", "Networking"],
    focusTags: ["C++20", "HTTP/2", "TLS", "HPACK", "Event Loop"],
    proofBadges: ["Public repo", "CI gates", "C++20", "HTTP/2", "TLS"],
    proofLine:
      "Public C++20 networking system with HTTP/2, HPACK, TLS application data, event-loop, and pool tests.",
    evidenceVisual: {
